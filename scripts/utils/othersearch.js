  ...
  .addEventListener("click", function () {
    ...
    for (i = 0; i < listIng.length; i++) {
      if (listIng[i].style.display != "") {
        ...
        document.querySelector("#ingredientList").setAttribute("class","active")
      }
    }
  });
...
function innerIngredientsFunction(){  
  ...
  document.querySelector("#ingredientList").innerHTML = "Ingredients" + allIngredientsList;
}
...
document.querySelector("#applianceList").innerHTML = "Appareils" + allAppliancesList;
}
...
function innerUstensilsFunction(){
  ...
  document.querySelector("#ustensilList").innerHTML ="Ustensiles" + allUstensilsList;
}

document.querySelector("#ingredientList").addEventListener("click",function(event){
  console.log(event.target.innerText.match(/(\n)/g))
  if(event.target.innerText.match(/(\n)/g) == null && event.target.innerText != "Ingredients"){
    console.log("valid")
...
document.querySelector("#applianceList").addEventListener("click",function(event){
  console.log(event.target.innerText.match(/(\n)/g))
  if(event.target.innerText.match(/(\n)/g) == null && event.target.innerText != "Appareils"){
    console.log("valid")
    const selectOptMark = document.createElement("div");
    selectOptMark.textContent = event.target.innerText;
    document.querySelector(".selectListOption").appendChild(selectOptMark);
...
document.querySelector("#ustensilList").addEventListener("click",function(event){
  console.log(event.target.innerText.match(/(\n)/g))
  if(event.target.innerText.match(/(\n)/g) == null && event.target.innerText != "Ustensiles"){
    console.log("valid")
    const selectOptMark = document.createElement("div");
    selectOptMark.textContent = event.target.innerText;
    document.querySelector(".selectListOption").appendChild(selectOptMark);
